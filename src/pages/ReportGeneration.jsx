...
function isSupported(item) {
  return item.contentType === "application/pdf" ||
    ...
    /.(png|jpe?g)$/i.test(item.fileName || "");
}
...
async function appendDocument(pdf, item) {
  ...
  const image = type === "image/png" || name.toLowerCase().endsWith(".png")
...
export default function ReportGeneration() {
  ...
  const clientDocuments = useMemo(
    () => selectedRequestId
      ? documents.filter((item) => item.requestId === selectedRequestId && item.clientId === selectedRequest?.clientId)
      : [],
    [documents, selectedRequestId, selectedRequest?.clientId]
  );

  const allDocuments = useMemo(
    () => [...clientDocuments, ...engineerDocuments],
    [clientDocuments, engineerDocuments]
  );
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      for (const item of allDocuments) {
        await appendDocument(pdf, item);
      }

      if (!pdf.getPageCount()) {
        throw new Error("There are no readable documents to export.");
      }
  ...
  return (
    <div className="container page-container report-page">
      <div className="page-header">
        <div>
          <span className="eyebrow">DOCUMENT REPORTS</span>
          <h1>Generate Report</h1>
          <p>Compile client documents and engineer-provided documents into one readable PDF.</p>
        </div>
      </div>
      ...
      <div className="report-layout">
        <section className="card report-control-card">
          ...
          <select value={selectedRequestId} onChange={(event) => setSelectedRequestId(event.target.value)}>
            ...
            {clientRequests.map((request) => (
              <option value={request.id} key={request.id}>
                {request.referenceNumber || request.id} — {request.association}
              </option>
            ))}
          </select>
          ...
          <div className="report-upload-box">
            <div>
              <strong>Add engineer documents</strong>
              <span>PDF, PNG, or JPG · maximum 600 KB each</span>
            </div>

            <label className="secondary-btn report-upload-button">
              {uploading ? "Adding..." : "Add files"}
              <input
                type="file"
                multiple
                accept=".pdf,.png,.jpg,.jpeg,application/pdf,image/png,image/jpeg"
                onChange={addEngineerDocuments}
                disabled={!selectedRequestId || uploading}
              />
            </label>
          </div>
          ...
          <button
            ...
            onClick={exportPdf}
            disabled={busy || !selectedRequestId || !allDocuments.length || unsupported.length}
          >
            {busy ? "Generating PDF..." : "Export Documents as PDF"}
          </button>
        </section>
        ...
        <section className="card report-documents-card">
          <div className="section-title">
            <div>
              <h3>Documents</h3>
              <p>{allDocuments.length} document{allDocuments.length === 1 ? "" : "s"} included</p>
            </div>
          </div>

          {!selectedRequestId && (
            <div className="empty-state">Select a client submission to view its documents.</div>
          )}

          {selectedRequestId && allDocuments.length === 0 && (
            <div className="empty-state">No documents have been added to this submission.</div>
          )}

          {selectedRequestId && clientDocuments.length > 0 && (
            <div className="report-document-group">
              <span className="report-group-label">Client documents</span>
              {clientDocuments.map((item) => (
                <div className="report-document-row" key={item.id}>
                  <div className="report-document-icon">{item.contentType === "application/pdf" ? "PDF" : "IMG"}</div>
                  <div className="report-document-name">
                    <strong>{item.fileName}</strong>
                    <span>{(item.fileSize / 1024).toFixed(0)} KB</span>
                  </div>
                  <span className="report-source">Client</span>
                </div>
              ))}
            </div>
          )}

          {selectedRequestId && engineerDocuments.length > 0 && (
            <div className="report-document-group">
              <span className="report-group-label">Engineer documents</span>
              {engineerDocuments.map((item) => (
                <div className="report-document-row" key={item.id}>
                  <div className="report-document-icon">{item.contentType === "application/pdf" ? "PDF" : "IMG"}</div>
                  <div className="report-document-name">
                    <strong>{item.fileName}</strong>
                  ...
                  <span className="report-source engineer">Engineer</span>
                  <button type="button" className="danger-btn small" onClick={() => removeEngineerDocument(item.id)}>
                    Remove
                  </button>
                </div>
              ))}
          ...
          {unsupported.length > 0 && (
            <div className="report-warning">
              Unsupported files are present. Export requires PDF, PNG, or JPG documents so every document can remain readable in the final PDF.
            </div>
          )}
        </section>
      </div>
