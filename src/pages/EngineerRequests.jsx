...
export default function EngineerRequests() {
  ...
  const confirmPending = () => {
    if (!note.trim()) {
      alert("An internal note is required.");
      return;
    }
    ...
    <div className="container page-container">
      <div className="page-header">
        <div>
          ...
          <p>Review requests and manage submitted documents.</p>
        </div>
      </div>

      <input
        className="search-input wide-search"
        placeholder="Search association or request ID..."
        value={search}
      ...
      <div className="request-list">
        {list.map((request) => {
          ...
          return (
            <div className="card request-review" key={request.id}>
              <div className="request-top">
                <div>
                  <span className="request-id">{request.referenceNumber || request.id}</span>
                  <h3>{request.association}</h3>
                  <p>{request.details}</p>
                  <small>Client: {client?.name || "Unknown"} · {client?.email || ""}</small>
                </div>
                <span className={`status ${String(request.status).toLowerCase().replaceAll(" ", "-")}`}>
                  {request.status}
                </span>
              </div>
              ...
              <div className="request-actions-row">
                {["Submitted", "Under Review"].includes(request.status) ? (
                  <div className="decision-actions">
                    <button className="primary-btn" onClick={() => setDecision(request, "Approved")}>Approve</button>
                    <button className="danger-btn" onClick={() => setDecision(request, "Rejected")}>Reject</button>
                    <button className="secondary-btn" onClick={() => setDecision(request, "Documents Pending")}>Documents Pending</button>
                  </div>
                ) : (
                  <div className="locked-note">
                    Current status: {request.status}. {request.notes && `Note: ${request.notes}`}
                  </div>
                )}

                <button className="danger-outline-btn" onClick={() => handleDelete(request)}>
                  Delete request
                </button>
              </div>
            </div>
      ...
      {selected && (
        <div className="modal-backdrop">
          <div className="modal card">
            ...
            <p>Add an internal note explaining what the client needs to provide.</p>
            <textarea
              rows="5"
              value={note}
            ...
            <div className="form-actions">
              ...
              <button className="primary-btn" onClick={confirmPending}>Confirm</button>
            </div>
          </div>
        </div>
      )}
